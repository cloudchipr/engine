...
export class GcpPriceCalculator {
  static async putVmPrices (items: Vm[], disks: Disks[], auth: any): Promise<void> {
    const skus = await GcpCatalogClient.collectAllStockKeepingUnits(auth)
    items.forEach((item) => {
      ...
      const ramPrice = skus.filter((vm) => vm.key === ramKey && vm.regions?.includes(item.getRegion()))[0]?.price
      const cpuPrice = skus.filter((vm) => vm.key === cpuKey && vm.regions?.includes(item.getRegion()))[0]?.price
  ...
  static async putDisksPrices (items: Disks[], auth: any): Promise<void> {
    const skus = await GcpCatalogClient.collectAllStockKeepingUnits(auth)
    items.forEach((item) => {
      const storage = skus.filter((st) => st.key === item.type && st.regions?.includes(item.getRegion()))[0]
  ...
  static async putEipPrices (items: Eip[], auth: any): Promise<void> {
    ...
    items.forEach((item) => {
      ...
      const network = skus.filter((nt) => nt.key === key && nt.regions?.includes(region))[0]
  ...
  static async putLbPrices (items: Lb[], auth: any): Promise<void> {
    ...
    items.forEach((item) => {
      ...
      const loadBalancer = skus.filter((lb) => lb.key === key && lb.regions?.includes(region))[0]
  ...
  static async putSqlPrices (items: Sql[], auth: any): Promise<void> {
    ...
    items.forEach((item) => {
      ...
      const ramPrice = skus.filter((db) => db.key === ramKey && db.regions?.includes(item.getRegion()))[0]?.price
      const cpuPrice = skus.filter((db) => db.key === cpuKey && db.regions?.includes(item.getRegion()))[0]?.price
      const storagePrice = skus.filter((db) => db.key === storageKey && db.regions?.includes(item.getRegion()))[0]?.price
